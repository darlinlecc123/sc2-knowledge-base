...
>
> 本知识库已经可以用于自然语言变量检索、LLM 上下文增强、代码定位和论文变量释义。当前推荐使用 **RAG**或者压缩包直接注入在线大模型，但 challenge v0.6 的唯一首次盲测仍有两项预设门槛未通过，因此项目状态保持 <code>conditional_go</code>，不宣称已经解决全部泛化问题。
